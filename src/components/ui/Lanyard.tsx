...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const baseImg = baseMap ? baseMap.image : null;
    const W = baseImg ? (baseImg.width || 512) : 512;
    const H = baseImg ? (baseImg.height || 512) : 512;
  ...
  useEffect(() => {
    if (texture) {
      const tex = texture as unknown as THREE.Texture;
